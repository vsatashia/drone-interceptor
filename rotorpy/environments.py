...
class Environment():
    ...
    def run(self,   t_final      = 10,       # The maximum duration of the environment in seconds
        ...
        start_time = clk.time()
        (time, state, control, flat, imu_measurements, imu_gt, mocap_measurements, state_estimate, exit, hoop_error, hoop_pose) = simulate(self.world,
        ...
        self.result = dict(time=time, state=state, control=control, flat=flat, imu_measurements=imu_measurements, imu_gt=imu_gt, mocap_measurements=mocap_measurements, state_estimate=state_estimate, exit=exit, hoop_error=hoop_error, hoop_pose=hoop_pose)
